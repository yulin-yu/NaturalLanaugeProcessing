...
# 返回gamm function of X
# wiki LDA, inference, 公式
# xx gamma function 的值
...
class VocabBuilder:
    ...
    def vocab(self, size=5000):          # 5000 最高词频
        """
...
class LdaTopicCounts: # 一篇文章数下来 词 词见过 出现多少次 没见过 0.1
    ...
    def change_prior(self, word, beta):   # 词的词频改了，正确词频率
        """
    ...
    def initialize(self, word, topic):   #这个topic现在有这个词
        """
    ...
    def get_normalizer(self, topic):    #
        ...
        """
        return self._beta_sum + self._normalizer[topic]   #被出# 多带

    def get_prior(self, word): #见过这个词律，返回词概率
        """
    ...
    def get_observations(self, topic, word):          #topic数多少
        """
    ...
    def word_in_topic(self, topic, word):      #除   # 用得最多
        """
    ...
    def report(self, vocab, handle, limit=40):    #打出来好看
        """
        ...
        for kk in self._normalizer:
            ...
            word = 0
            #new =sorted(self._topic_term.values(), reverse=True)  # add ranking
            for ww in self._topic_term[kk]:
                handle.write("%0.5f\t%0.5f\t%0.5f\t%s\n" % \
                             (self.word_in_topic(kk, ww),
                              self.get_observations(kk, ww),   # 按照 get_observation or word in topic 进行排序 # 放在字典里面
                              self.get_prior(ww),
                              vocab[ww]))
                ...
                if word > limit:
                    break
    #new function
    def reportwithrank(self, vocab, handle, limit=40):    #打出来好看。# change ranking
        """
        Create a human readable report of topic probabilities to a file.
        """
        newlist = []
        for kk in self._normalizer:
            ...
            for ww in self._topic_term[kk]:
            	insidelist = [self.word_in_topic(kk, ww),self.get_observations(kk, ww),self.get_prior(ww),vocab[ww]]
            	newlist.append(insidelist)
            revlist = sorted(newlist, reverse = True)
            for a in revlist:
            	#print(a)
            	print(a[0])
            	#for a in i:
...
class Sampler:   #优化过程  #alpha，sample 概率，是个topic alpr，beta = alphr。。。。sample g dis，除以他们的的sum，加起来=1， 跟着alpha打下    # alpha更新
    ...
    def add_doc(self, doc, vocab, doc_id = None,      # get doc 反过来，数词     #doc——id：文章index 第几篇
                token_limit=-1):
        """
    ...
    def change_topic(self, doc, index, new_topic):     #文章拿掉 设置-1， 把文章放回去 设计不是-1	
        """
        ...
        if old_topic != -1:
            assert new_topic == -1    #old topic 拿掉词频
            # 拿掉一个sample
            #每个topic有多少文章
            
            # TODO: Add code here to keep track of the counts and
            # assignments 
            #self._doc_assign[doc][index]-= 1    # document topic 
            #self._doc_counts[doc][old_topic] -= 1      
            #self._num_topics -= 1            # topic count
            #self._doc_tokens -= 1          # document lengths
            self._doc_assign[doc][index] = -1
            self._doc_counts[doc][new_topic] += -1   #(2)
            self._topics.change_count(new_topic, term, -1) #(1) (3)
			#length = len(self._doc_assign[doc_id])-1

# don't need to change doc_length? 

        if new_topic != -1:
            assert old_topic == -1     #new topic 加入词频

            # TODO: Add code here to keep track of the counts and
            # assignments
            #self._topics.initialize(term,new_topic)
            # same, use tokens or assign?
            #self._doc_tokens[doc][index]
            # need: choose new topic: 
    ...
    #def change_count(self, topic, word, delta):   #
     ...
     #   self._finalized = True
      ...
      #  self._topic_term[topic][word] += delta   (1)
      #  self._normalizer[topic] += delta        (3)
    ...
    def run_sampler(self, iterations = 100):     #gip sample
        """
    ...
    def report_topics(self, vocab, outputfilename, limit=100):    #打出来好看的 # change print 100 topics
        """
...
#### DOODODODODDO
        mytopicsile = open("my-topics.txt", 'w')
        self._topics.reportwithrank(vocab, mytopicsile, limit)

        ##### TODO： ave your results asmy-topics.txt which contains the one hundred most probable words per topic, 
        #sorted with the most probable first. You’ll run to runyour implementation for at least for 1000 iterations. 
        # sample_probs larger 大

        tracktime = open("time.txt", 'w')   # track time done
        ### TODO: Keep track of how long it took in seconds forthe model to finish 1000 iterations (you’ll need this)
        sumtime = []
        for it in range(0, len(self._lhood)):
            tmp = float(self._time[it])
            sumtime.append(tmp)
        sumsum = sum(sumtime)
        tracktime.write(str(sumsum))
        tracktime.close()

    def sample_probs(self, doc_id, index):        #给一篇文章词频，有多大概率 告诉sampledoc，当我把第一篇文章，我应该重新计算，不同对应prob，dict，key = topic index，value = prob， 扔文章不扔topic
        """
        ...
        for kk in range(self._num_topics):
            ...
            # uniform probability.
            #当你什么都不知道的时候 用uniform
            #sample_probs[kk] = 1.0 / float(self._num_topics)  #老师写错了
            # 

            prob_word_given_topic = self._topics.word_in_topic(kk,term)  # done
            
            doc_num = self._doc_counts[doc_id][kk]
            alpha = self._alpha[kk]
            Nalpha = sum(self._alpha)  #N = number of topic
            lengthdoc = len(self._doc_assign[doc_id])-1  # take out the -1 index

            prob_topic_given_doc = (doc_num+alpha)/(lengthdoc + Nalpha) # wried 
            sample_probs[kk] = prob_word_given_topic*prob_topic_given_doc
    ...
    def sample_doc(self, doc_id, debug=False):   #给一篇文章 每个docu 一篇文章 这个文章 好几个topic，第一个topic拿掉 sample 一个topic，
        """
    ...
    def lhood(self):     #越来越高 看对错的 不管
        val = self.doc_lhood() + self.topic_lhood()
        return val

    def doc_lhood(self):   #越来越高 看对错的 不管
...
def tokenize_file(filename): #一篇文章
...
if __name__ == "__main__":
    ...
    vocab_scanner = VocabBuilder(args.language)#把数据放vocalBuilder class里
    ...
    for ii in files:
        vocab_scanner.scan(tokenize_file(ii)) #清理词汇 

    # Initialize the documents
    vocab = vocab_scanner.vocab(args.vocab_size)     #+ 扫出top 5000词
    print((len(vocab), vocab[:10]))                  #看看有多少次 词长什么样子
    lda = Sampler(args.num_topics, vocab)          #把数据放在sample class
    for ii in files:
        lda.add_doc(tokenize_file(ii), vocab)    #get id get index

    lda.run_sampler(args.num_iterations)     #giff samplling # sample_doc <- change topic sample probablity
    lda.report_topics(vocab, args.output)   #打出来好看


### Reference:
#Beaugureau, M. (2015, May 12). Topic modeling for the newbie. Retrieved from https://www.oreilly.com/ideas/topic-modeling-for-the-newbie?fbclid=IwAR0LQKAnXQtURSyWwVk7Zy9FcCSRiR_nSLIbCei4ahbf9qlkNynIhG9KAiQ
