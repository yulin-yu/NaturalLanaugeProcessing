import numpy as np
import torch
from feature_extraction import punc_pos, pos_prefix
...
def compute_dependencies(model, device, data, dataset):
    ...
    for batch_start in range(0, len(sentences), dataset.model_config.batch_size):

        # The sentences we'll process
        batch_sentences = sentences[batch_start:batch_start+dataset.model_config.batch_size]
        ...
        while enable_count > 0:
            ...
            predictions = model(word_inputs_batch,
